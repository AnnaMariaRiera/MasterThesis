%% Random motion and balanced circular formation together
%%
...
%    - v_max = 0.13 m/s
%% Problems:
%  - Avoidance not working properly when vi<0 for circular robot i
%%
clear all
clc
...
dim1 = 1.2; dim2 = 1.2; % dim tauler (40,40)
theta = 0:pi/50:2*pi;
...
Nc = 2; % number of circular cars (3)
...
Nr = 3; % number of circular cars (4)
s = v_max*0.6; % speed v_max*0.6 ok
lambd = 0.8; % -> 0.8 ok
...
function [G1,G2] = setTarget(G0,tag)
    if strcmp(tag,'static') 
        G1 = @(t) G0(1); 
        G2 = @(t) G0(2); % static G
    elseif strcmp(tag,'circular') 
        r_g = 20; omega_g = 0.01;
        G1 = @(t) G0(1) + r_g*cos(omega_g*t-pi); 
        G2 = @(t) G0(2) + r_g*sin(omega_g*t-pi);
...
function [t_all,x_all,F] = solveAll(x0,Nr,Nc,T,G1,G2,s,d_detect,dim1,dim2,epsilon,k_v,k_w,v_max,w_max,phiR,Phi_r)
    ...
    tspan = 0:0.1:T;  % abans: tspan = [0 T];
    fprintf('F created\n')
    
    opts = odeset('RelTol',1e-8,'AbsTol',1e-8,'MaxStep',epsilon/(15*v_max));  % els dos haurien de ser 1e-12
...
function P = calculatePhi(T,lambd,tag_plot,w_max)
    ...
    M = length(ts);
    %phis = 2*pi*rand(M,1);
    w_reduction = 1; % (abans era 0.7)
    phis = -(w_max*w_reduction) + 2*(w_max*w_reduction)*rand(M,1);
...
function [bearing,flag] = Dists(x,Nr,Nc,dim1,dim2,epsilon,d_detect)
    ...
    % D    IF N=1, THEN D=[] AND IT HAS LESS ELEMENTS!!!
    D2 = [D  db'.*0.8  dl'.*0.8  dt'.*0.8  dr'.*0.8];  % add distance to bndies to D (dim (Nc+Nr,Nc+Nr+4))
                   % I put more "weight" to the bdies so the robots do not cross them 
                   % (0.8 works, but other numbers could have been chosen as well)
    ...
    [I,J] = find(D2'<=d_detect+0.1); % 0.1 always works, 0.05 only sometimes
...
function [] = plot_vc(t,v,tag,Nc,T,d_detect,s,lambd,v_max,phi)
    ...
    if Nc>0
        ...
        xlabel('Time (s)', 'FontSize', 12);
        ylabel(tag, 'FontSize', 12);
        %legend("Car 1", "Car 2", "Car 3", "Expected final v", 'Location', 'southeast');
        %legend("Car 1", "Car 2", "Car 3", 'Location', 'southeast');
        title(['T=',num2str(T),', ddetect=',num2str(d_detect),', s=',num2str(s),...
            ', lambd=',num2str(lambd)]);
        set(gca,'FontSize',12)
        hold off
...
function [cells_2d_plot] = count_cells_plot(M,xr,dim1,dim2,Nr)
    cells = zeros(M+1);
    X_grid = 0:dim1/M:dim1; % M+1 termes
    Y_grid = 0:dim2/M:dim2; % M+1 termes
    dim = size(xr);
    found = 1;
...
function [] = plot_freqs(M,cells,dim1,dim2)
    ...
    colorbar;
    set(gca,'FontSize',12)
    view(2); % si traiem això és una superfície 3d
end
