# Circular motion using UMC2 with 3 vehicles (4th vehicle is the center of the formation)
...
count=0
distance = 0.2  # radius of the formation
kv = 1  
wv = 1   # kw
vmax = 0.13
wmax = 4
...
while (True):
	...
	if (count<p):
		if np.all(ids != None):  # if all ids are different to "None"
			if (all(i in ids for i in epuckids)):  # if all epucks' ids are being detected
				for i in range(0, n):
					tvecs[i] = (tvec[np.where(ids == epuckids[i])][0])  # TVECS=POSITION  -> tvecs[n-1][1, 2 o 3] position of target
					...
					b = ((math.atan2(tvecs[n - 1][1] - tvecs[i][1], tvecs[n - 1][0] - tvecs[i][0]) + 2 * math.pi) % (2 * math.pi) - (angles[i][2] + 2 * math.pi) % (2 * math.pi) +2*math.pi)%(2*math.pi)  # alpha_i (target G corresponds to the last epuck (n-1))
